...
func TestTransportEntropyBudgetStaysLimitedAfterReachingZero(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	defer clientConn.Close()
	defer serverConn.Close()
	...
	// The first packet used the whole budget. Zero now means "nothing left",
	// not "unlimited".
	if err := transport.Send([]byte("second")); err == nil {
		t.Fatal("expected exhausted entropy budget to stay exhausted")
	}
}
